...
static const NSTimeInterval kDefaultRequestTimeout = 40;
static const NSInteger kNumberOfSamples = 5;
static const long long kMinSpace = 500 * 1024 * 1024;
...
- (void)start
{
    ...
    //由于cdn不支持Range,上面屏蔽了的代码改成下去的
    if ([fm fileExistsAtPath:self.pathToFile]) {
        [fm removeItemAtPath:self.pathToFile error:nil];
    }
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if ([self notifyFromCompletionWithNotEnoughFreeDisk]) {
        return;
    }
...
-(BOOL)notifyFromCompletionWithNotEnoughFreeDisk{

    long long expected = @(self.expectedDataLength).longLongValue;
    long long freeSpace = [TCBlobDownloader freeDiskSpace].longLongValue;
    if ((freeSpace < expected * 20 || freeSpace < kMinSpace) && expected != -1) {
        NSError *error = [NSError errorWithDomain:TCBlobDownloadErrorDomain
